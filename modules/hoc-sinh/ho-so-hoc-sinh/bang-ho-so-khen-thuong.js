SchoolityApp.controller('bangHoSoKhenThuong',function($scope,$rootScope,$http){
    $http.get('/shared/json/khenthuong.json').then(function(res){
        $scope.dsKhenThuong = res.data;
    })
    // Watch giá trị biến checkAll
    $scope.selectedItem= new Array();
    $scope.$watch('checkAll',function(val){
        if(val){
            $scope.selectedItem = [...$scope.dsKhenThuong];
        }else{
            $scope.selectedItem = new Array();
        }
    });
    ...
    $scope.Delete = function(){
        $scope.dsKhenThuong = $scope.dsKhenThuong.filter(item => !$scope.selectedItem.includes(item));
    }
