SchoolityApp.controller('bangHoSoBaoLuu',function($scope,$rootScope,$http){
    $http.get('/shared/json/baoluu.json').then(function(res){
        $scope.dsBaoLuu = res.data;
    })
    // Watch giá trị biến checkAll
    $scope.selectedItem= new Array();
    $scope.$watch('checkAll',function(val){
        if(val){
            $scope.selectedItem = [...$scope.dsBaoLuu];
        }else{
            $scope.selectedItem = new Array();
        }
    });
    ...
    $scope.Delete = function(){
        $scope.dsBaoLuu = $scope.dsBaoLuu.filter(item => !$scope.selectedItem.includes(item));
    }
